...
var invertValueSlider = document.getElementById("invertValueSlider");
...
invertValueSlider.addEventListener("input", function(e){
	//infoLbl.innerText = this.value;
	// invert colors in the canvas
...
invertValueSlider.addEventListener("change", function(e){
	
	// If no layers we cannot apply 
	if(layerStack.length <= 0){
		invertValueSlider.value = 0;
		editsMade = false;
		return;
	}
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(invertValueSlider.value) != 0){
	}else{
		//alert("no edits");
		return;
	}
	...
	let invertEditEntry = {
		edit: "invert", 
		edit_label: "Invert Colors", 
		value: invertValueSlider.value
	};
	*/
	
	var invertEditEntry = new Object();
	invertEditEntry.edit = "invert";
	invertEditEntry.edit_label = "Invert Colors";
	invertEditEntry.value = invertValueSlider.value;
	
	invertValueSlider.value = 0;
	
	// add to the blur edit stack
	editStack.add(invertEditEntry);
...
invertValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let invertEditEntry = {edit: "blur", radius: invertValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			invertCanvas.width = layerStack[j].canvas.width; 
			invertCanvas.height = layerStack[j].canvas.height; 
			let invertCtx = invertCanvas.getContext("2d");
			...
			// 2. Draw the layer image to the invertCanvas again with blur filter applied
			invertCtx.filter = "invert(" + invertValueSlider.value + "%)";		
			//invertCtx.drawImage(image, 0, 0);
			//invertCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//invertCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			invertCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(invertCtx, layerStack[j].cumulativeNegatives);
			...
			tempEditorCtx.drawImage(invertCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
			
			var layerCtx =  layerStack[j].canvas.getContext("2d");
			//layerCtx.drawImage(eraserCanvas, 0, 0);	
			layerCtx.drawImage(invertCanvas, 0, 0);			
