...
var blurValueSlider = document.getElementById("blurValueSlider");
...
blurValueSlider.addEventListener("input", function(e){
...
blurValueSlider.addEventListener("change", function(e){
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(blurValueSlider.value) != 0){
	}else{
		// alert("no edits");
		return;
	}
	...
	let blurEditEntry = {
		edit: "blur", 
		edit_label: "Blur Layer", 
		radius: blurValueSlider.value
	};
	*/

	var blurEditEntry = new Object();
	blurEditEntry.edit = "blur";
	blurEditEntry.edit_label = "Blur Layer";
	blurEditEntry.radius = blurValueSlider.value;
	
	
	blurValueSlider.value = 0;
	
	// add to the blur edit stack
	editStack.add(blurEditEntry);
...
blurValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let blurEditEntry = {edit: "blur", radius: blurValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			// 2. Draw the layer image to the blurredCanvas again with blur filter applied
			blurredCtx.filter = "blur(" + blurValueSlider.value + "px)";			
			//blurredCtx.drawImage(image, 0, 0);
			//blurredCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//blurredCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			blurredCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(blurredCtx, layerStack[j].cumulativeNegatives);
			...
			editorCtx.drawImage(blurredCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
