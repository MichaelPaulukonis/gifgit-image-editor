...
var hueValueSlider = document.getElementById("hueValueSlider");
...
hueValueSlider.addEventListener("input", function(e){
...
hueValueSlider.addEventListener("change", function(e){
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(hueValueSlider.value) != 0){
	}else{
		// alert("no edits");
		return;
	}
	...
	let hueEditEntry = {
		edit: "hue", 
		edit_label: "Hue", 
		value: hueValueSlider.value
	};
	*/
	
	var hueEditEntry = new Object();
	hueEditEntry.edit = "hue";
	hueEditEntry.edit_label = "Hue";
	hueEditEntry.value = hueValueSlider.value;
...
hueValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let hueEditEntry = {edit: "blur", radius: hueValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			huedCanvas.width = layerStack[j].canvas.width; 
			huedCanvas.height = layerStack[j].canvas.height; 
			...
			// 2. Draw the layer image to the huedCanvas again with blur filter applied
			huedCtx.filter = "hue-rotate(" + hueValueSlider.value + "deg)";				
			//huedCtx.drawImage(image, 0, 0);
			//huedCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//huedCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			huedCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(huedCtx, layerStack[j].cumulativeNegatives);
			...
			tempEditorCtx.drawImage(huedCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
			
			var layerCtx =  layerStack[j].canvas.getContext("2d");
			//layerCtx.drawImage(eraserCanvas, 0, 0);	
			layerCtx.drawImage(huedCanvas, 0, 0);			
