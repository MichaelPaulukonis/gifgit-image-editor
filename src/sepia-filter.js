...
var sepiaValueSlider = document.getElementById("sepiaValueSlider");
...
sepiaValueSlider.addEventListener("input", function(e){
	//infoLbl.innerText = this.value;
	// change the sepia of the canvas
...
sepiaValueSlider.addEventListener("change", function(e){

	// If no layers we cannot apply 
	if(layerStack.length <= 0){
		sepiaValueSlider.value = 0;
		editsMade = false;
		return;
	}	
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(sepiaValueSlider.value) != 0){
	}else{
		//alert("no edits");
		return;
	}
	...
	let hueEditEntry = {
		edit: "sepia", 
		edit_label: "Sepia", 
		value: sepiaValueSlider.value
	};
	*/
	
	var hueEditEntry = new Object();
	hueEditEntry.edit = "sepia";
	hueEditEntry.edit_label = "Sepia";
	hueEditEntry.value = sepiaValueSlider.value;

	// add to the blur edit stack
	editStack.add(hueEditEntry);
...
sepiaValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let hueEditEntry = {edit: "blur", radius: sepiaValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			sepiaCanvas.width = layerStack[j].canvas.width; 
			sepiaCanvas.height = layerStack[j].canvas.height; 
			...
			// 2. Draw the layer image to the sepiaCanvas again with blur filter applied
			sepiaCtx.filter = "sepia(" + sepiaValueSlider.value + "%)";			
			//sepiaCtx.drawImage(image, 0, 0);
			//sepiaCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//sepiaCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			sepiaCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(sepiaCtx, layerStack[j].cumulativeNegatives);
			...
			tempEditorCtx.drawImage(sepiaCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
			
			var layerCtx =  layerStack[j].canvas.getContext("2d");
			//layerCtx.drawImage(eraserCanvas, 0, 0);	
			layerCtx.drawImage(sepiaCanvas, 0, 0);			
