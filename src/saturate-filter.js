...
var saturationValueSlider = document.getElementById("saturationValueSlider");
...
saturationValueSlider.addEventListener("input", function(e){
...
saturationValueSlider.addEventListener("change", function(e){
	
	// If no layers we cannot apply 
	if(layerStack.length <= 0){
		saturationValueSlider.value = 100;
		editsMade = false;
		return;
	}	
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(saturationValueSlider.value) != 100){
	}else{
		//alert("no edits");
		return;
	}	
	...
	/*
	let saturateEditEntry = {
		edit: "saturate", 
		edit_label: "Saturate", 
		value: saturationValueSlider.value
	};
	*/
	
	var saturateEditEntry = new Object();
	saturateEditEntry.edit = "saturate";
	saturateEditEntry.edit_label = "Saturate";
	saturateEditEntry.value = saturationValueSlider.value;
	
	// add to the blur edit stack
	editStack.add(saturateEditEntry);
...
saturationValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let saturateEditEntry = {edit: "blur", radius: saturationValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			saturatedCanvas.width = layerStack[j].canvas.width; 
			saturatedCanvas.height = layerStack[j].canvas.height; 
			...
			// 2. Draw the layer image to the saturatedCanvas again with blur filter applied
			saturatedCtx.filter = "saturate(" + saturationValueSlider.value + "%)";				
			//saturatedCtx.drawImage(image, 0, 0);
			//saturatedCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//saturatedCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			saturatedCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(saturatedCtx, layerStack[j].cumulativeNegatives);
			...
			tempEditorCtx.drawImage(saturatedCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
			
			var layerCtx =  layerStack[j].canvas.getContext("2d");
			//layerCtx.drawImage(eraserCanvas, 0, 0);	
			layerCtx.drawImage(saturatedCanvas, 0, 0);			
