...
if (localStorage.getItem("blur_brush_settings") === null) {
	updateBrush();
}else{
	//alert(localStorage.getItem("background_eraser_settings"));
	let brushSettings = JSON.parse(localStorage.getItem("blur_brush_settings"));
...
workspaceContainerDiv.addEventListener("mouseup", function(e){
	...
	if(transformSelection){
		...
		// FINISH THE BLUR TOOL APPLICATION
...
function saveBrush(){
	...
	localStorage.setItem("blur_brush_settings", JSON.stringify(brushSettings));	
...
function stopBrushPaint(){
	...
	if(clickX.length > 0){
		...
		*/
		...
		var blurToolEditEntry = new Object();
		blurToolEditEntry.edit = "blur_tool";
		blurToolEditEntry.edit_label = "Blur Tool";
		blurToolEditEntry.clickX = clickX;
		blurToolEditEntry.clickY = clickY;
		blurToolEditEntry.clickDrag = clickDrag;
		blurToolEditEntry.brushSize = brushSize;
		blurToolEditEntry.opacity = brushAlpha;
		blurToolEditEntry.blur = brushBlur;
	
		// add to the crop edit stack
		editStack.add(blurToolEditEntry);
	}
...
function updateToolCanvas(){
	...
	if(transformSelection){
		...
		toolDrawCtx.arc(workspaceMouseX, workspaceMouseY, (globalZoom/100) * brushSize/2, 0, 2 * Math.PI);
		toolDrawCtx.stroke();
	}
	toolDrawCtx.restore();
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex && clickX.length > 0){
			...
			//blurStrokesCanvas.width = editorCanvas.width;
			//blurStrokesCanvas.height = editorCanvas.height;			
			blurStrokesCanvas.width = layerStack[j].canvas.width;
			blurStrokesCanvas.height = layerStack[j].canvas.height;
			var blurStrokesCtx = blurStrokesCanvas.getContext("2d");

			blurStrokesCtx.filter = "blur(1px)";
			blurStrokesCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			...
			blurStrokesCtx.drawImage(eraserCanvas, 0, 0);	

			
			// clip the blur brush region
			EditorUtils.clipSelectionRegion(blurStrokesCtx);	

			/*			

			tempEditorCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);				

			tempEditorCtx.drawImage(blurStrokesCanvas, 0, 0);				
			//tempEditorCtx.drawImage(eraserCanvas, 0, 0);	
			*/
			
			if(editingLayerMask == 0){
				// Editing to the layer canvas
				var layerCtx =  layerStack[j].canvas.getContext("2d");
				//layerCtx.drawImage(eraserCanvas, 0, 0);	
				// layerCtx.drawImage(blurStrokesCanvas, 0, 0);
				layerCtx.drawImage(blurStrokesCanvas, - layerStack[layerIndex].cumulativeNegatives.x, - layerStack[layerIndex].cumulativeNegatives.y);	

			}else if(editingLayerMask == 1){
				// Editing to the raster mask
				
			}else if(editingLayerMask == 2){
				// Editing to the vector mask
