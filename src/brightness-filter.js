...
var brightnessValueSlider = document.getElementById("brightnessValueSlider");
...
brightnessValueSlider.addEventListener("input", function(e){
...
brightnessValueSlider.addEventListener("change", function(e){
...
resetEditBtn.addEventListener("click", function(e){
	brightnessValueSlider.value = 100;
	updateEditorCanvas();
	editsMade = false;
});
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(brightnessValueSlider.value) != 100){
	}else{
		//alert("no edits");
		return;
	}
	...
	/*
	let brightnessEditEntry = {
		edit: "brightness", 
		edit_label: "Brightness", 
		value: brightnessValueSlider.value
	};
	*/
	
	var brightnessEditEntry = new Object();
	brightnessEditEntry.edit = "brightness";
	brightnessEditEntry.edit_label = "Brightness";
	brightnessEditEntry.value = brightnessValueSlider.value;
	
	brightnessValueSlider.value = 100;
	
	// add to the blur edit stack
	editStack.add(brightnessEditEntry);
...
brightnessValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let brightnessEditEntry = {edit: "blur", radius: brightnessValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			// 2. Draw the layer image to the brightenedCanvas again with blur filter applied
			brightenedCtx.filter = "brightness(" + brightnessValueSlider.value + "%)";			
			...
			brightenedCtx.drawImage(layerStack[j].canvas, 0, 0);
