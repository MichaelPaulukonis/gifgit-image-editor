...
var contrastValueSlider = document.getElementById("contrastValueSlider");
...
contrastValueSlider.addEventListener("input", function(e){
...
contrastValueSlider.addEventListener("change", function(e){
...
applyEditBtn.addEventListener("click", function(e){
	...
	if(editStack.stack.length > 0 && parseInt(contrastValueSlider.value) != 100){
	}else{
		//alert("no edits");
		return;
	}
	...
	/*
	let contrastEditEntry = {
		edit: "contrast", 
		edit_label: "Contrast", 
		value: contrastValueSlider.value
	};
	*/
	
	var contrastEditEntry = new Object();
	contrastEditEntry.edit = "contrast";	
	contrastEditEntry.edit_label = "Contrast";	
	contrastEditEntry.value = contrastValueSlider.value;	


	// add to the blur edit stack
	editStack.add(contrastEditEntry);
...
contrastValueSlider.addEventListener("change", function(e){
	//infoLbl.innerText = this.value;
	// apply the blur
	let contrastEditEntry = {edit: "blur", radius: contrastValueSlider.value};
...
function updateEditorCanvas(){
	...
	for(let j = 0; j < layerStack.length; j++){
		...
		if(j == layerIndex){	
			...
			contrastedCanvas.width = layerStack[j].canvas.width; 
			contrastedCanvas.height = layerStack[j].canvas.height; 
			...
			// 2. Draw the layer image to the contrastedCanvas again with blur filter applied
			contrastedCtx.filter = "contrast(" + contrastValueSlider.value + "%)";			
			//contrastedCtx.drawImage(image, 0, 0);
			//contrastedCtx.drawImage(image, layerStack[0].cumulativeNegatives.x, layerStack[0].cumulativeNegatives.y);
			//contrastedCtx.drawImage(layerStack[j].canvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			contrastedCtx.drawImage(layerStack[j].canvas, 0, 0);
			
			
			// 3. Perform Clipping - if there is a selection
			EditorUtils.clipSelectionRegion(contrastedCtx, layerStack[j].cumulativeNegatives);
			...
			tempEditorCtx.drawImage(contrastedCanvas, layerStack[j].cumulativeNegatives.x, layerStack[j].cumulativeNegatives.y);
			*/
			
			var layerCtx =  layerStack[j].canvas.getContext("2d");
			//layerCtx.drawImage(eraserCanvas, 0, 0);	
			layerCtx.drawImage(contrastedCanvas, 0, 0);			
